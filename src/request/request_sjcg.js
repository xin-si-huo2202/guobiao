...
if (process.env.NODE_ENV == 'production') {
    baseURL = 'http://41.211.252.150:18889' + '/jeecg-boot'
} else {
...
service.interceptors.request.use(
    config => {
        ...
        if(config.method=='get'){
            config.baseURL = config.ip+'/jeecg-boot'
        }else{
            ...
            if(!config.data.ip){
                ...
                config.baseURL = config.data.ip+'/jeecg-boot'
            }
        ...
        if (config.data && config.data != undefined && config.url !== "/statisticsStatementController/statisticsStatement" && config.url !== "/statisticsStatementController/statisdc" && config.url !== "/search_face/imgsurl") {
        ...
        if(!config.data.tokens){
            var headerss = Object.assign({}, config.headers, {
                'X-Access-Token': config.tokens
            })
        }else{
            var headerss = Object.assign({}, config.headers, {
                'X-Access-Token': config.data.tokens
            })
        }

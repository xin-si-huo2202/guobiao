...
import { v4 } from 'uuid';
import { gb_channels,gb_login,gb_start,gb_stop,gb_devicelist,gb_control,ss} from '@/api/video2.js'
import { getVideoUrl ,hikControl,getVideoUrl2,hikControl2 } from '@/api/hkvideo.js'
	...
	methods: {
		...
		async ptzCamera(leftRight, upDown, zoom) {
		    ...
		    try {
				...
				await gb_control(control_data,this.jc_data.bs);
		...
		async gettongdao(data,index){
            ...
            try {
				...
				await gb_login(loginParam,this.jc_data.bs)

				this.$cookies.set("session", {"username": 'admin'}) ;

				
				var res = await gb_channels(data.ip,this.jc_data.bs);
		...
		async S_gb_start(data){
            ...
            var da = {
				...
				if(this.jc_data.bs == 'videoXichuan'){
					vjk = '41.206.1.31'
				}else if(this.jc_data.bs == 'videoTb'){
					vjk = '41.210.0.123'
				}else if(this.jc_data.bs == 'videoThjc'){
					vjk = '41.208.9.228'
				}else if(this.jc_data.bs == 'videoTjc'){
					vjk = '41.211.252.154'
				}else if(this.jc_data.bs == 'videoNz'){
					vjk = '41.201.0.218'
				}else if(this.jc_data.bs == 'videoNxjc'){
					vjk = '41.205.32.246'
				}else if(this.jc_data.bs == 'videoSqjc'){
					vjk = '41.207.0.185'	
				}


                let urls= `ws://${vjk}:30001/rtp/${data.deviceId}/${data.channelId}.flv`;

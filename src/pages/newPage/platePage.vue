    ...
    <div class="portrait-main-conent">
      ...
      <div class="right-con" id="con-list">
        <template v-if="dataList && dataList.length > 0">
          <div class="con-first" :class="isJSStatus ? 'add_con-first' : ''">
            <div
              class="con-box"
              :id="'chepai-scroll' + index"
              @scroll="scrollEvent"
            >
              <template v-if="isJSStatus">
                <div
                  class="tab-item tab-item-one"
                  v-for="(itemL, indexL) in jiansuoWeizhiList"
                  :key="itemL.id"
                >
                  <div
                    class="tab-item-top"
                    @click="lookDetailPlate(itemL, indexL, jiansuoWeizhiList)"
                  >
                    <el-image
                      style="width: 140px; height: 160px; background: #eee"
                      :src="productionImage(itemL.bigImageUrl)"
                      fit="contain"
                    >
                      <!--  @click="lookDetailCarCaiji(itemL, indexL)" -->
                    </el-image>
                ...
                >
                  <div class="tab-item-top">
                    <el-image
                      ...
                      :src="productionImage(itemcj.bigImageUrl)"
          ...
          <!--   -->
          <div style="position: fixed; bottom: 0;background: #ebebeb;width: 100%; display: flex;justify-content: center;align-items: center;" v-if="!isJSStatus">
            <!-- 分页 -->
            <el-pagination
    ...
    async getAllRX(val = 1) {
      ...
      try {
        var res = await gyxySubImageInfo({
          ...
          sysOrgCode: "",
          policeCarNumber: this.paramSel.gatherCar
            ? this.paramSel.gatherCar
            : "",
        });

<template>
  <div class="Map-Video">
    <Map />
    <div class="check-box">
      <el-checkbox-group v-model="checkList" @change="chax()">
        <el-checkbox
          :label="item.label"
...
export default {
  ...
  methods: {
    chax() {
      console.log(this.checkList, "this.checkListthis.checkList++++");

      PubSub.publish("clear_jldt");
    ...
    async selAllDepartOtherInfo() {
      ...
      if (res.result) {
        let dep_list = res.result;

        PubSub.publish("add_jldt_dep", dep_list);
      }
    },
    async gyxyMapFlag() {
      ...
      if (res.result.records) {
        let lk_list = res.result.records;

        PubSub.publish("add_jldt_lk", lk_list);
      }
    },
    qie(type) {
      this.tabs = type;

...
import { getSerialNumberWarning } from "@/api/warningInfo.js";
...
export default {
  ...
  methods: {
    ...
    xuanze(item) {
      // item.active=true
      var p = Object.assign({}, item, { isPhone: true }, { change: v4() });
      this.gujid = p;
    },
    async getc() {
      console.log(this.fromCar, "c车牌号");
      console.log(this.startTime, "c车牌号");
      console.log(this.endTime, "c车牌号");
      var loading_lo;
      try {
        loading_lo = startLoding("#Tracetracing_bot");
        console.log("手机串号功能执行");
        var res = await getSerialNumberWarning({
          fromCar: this.fromCar,
          pageNo: "",
          pageSize: "",
          selectType: "",
          departId: "",
        ...
        console.log(res, "=========根据车牌去查找");
        if (res.result && res.result.records) {
          this.tracing_list = [...res.result.records];
        }
      } catch (err) {
  ...
  mounted() {
    ...
    this.fromCar = this.$route.query.fromCar;

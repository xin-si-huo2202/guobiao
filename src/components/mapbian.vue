<template>
  <div class="map_obj_box">
    ...
    <el-collapse accordion class="nima">
      <!--<el-collapse-item>
        <template slot="title">
          &nbsp; &nbsp; &nbsp; &nbsp; 区域列表<i
      ...
      <!--<el-collapse-item>
        ...
        <el-table :data="pointData" height="500" style="width: 100%">
          <el-table-column prop="id" label="打卡点"> </el-table-column>

          <el-table-column fixed="right" label="操作" width="120">
            <template slot-scope="scope">
              <el-button
                @click.native.prevent="deleteDaka(scope.$index, pointData)"
                type="text"
                size="small"
              >
                删除
      ...
      <el-collapse-item>
        ...
        >
          <el-table-column prop="gridName" label="区域名称" width="180">
          </el-table-column>
          <el-table-column prop="name" label="操作" width="180">
            <template slot-scope="scope">
              <el-button
                type="primary"
                @click="addAllData(scope.$index, scope.row)"
                >添加</el-button
              >
              <el-button
                type="primary"
                @click="lookAllData(scope.$index, scope.row)"
                >查看</el-button
              >
            </template>
          </el-table-column>
    ...
    >
      <el-form :model="form">
        ...
        <el-form-item label="区域颜色" :label-width="formLabelWidth">
          <el-color-picker v-model="form.color1"></el-color-picker>
          <!--<el-select v-model="form.color" placeholder="请选择区域颜色">
        ...
        <!--<el-form-item label="图片上传" :label-width="formLabelWidth">
          <img
            src="../../public/image/img_upload.png"
            alt=""
            style="height: 150px"
            @click="handler_upload"
          />
        </el-form-item>-->
      </el-form>
...
export default {
  ...
  methods: {
    ...
    quxiao() {
      ...
      this.mapdui.removeOverlay(this.newOverlay.overlay);
    ...
    async quding() {
      ...
      console.log(this.gridtype, "type");
      this.dialogFormVisible = false;
      if (this.gridtype == "1") {
        //放空区
        var id = this.pustAllData;
        //var param = new FormData();
        //param.append("id", id);
        //param.append("gridArea", JSON.stringify(this.path));
        //param.append("color", this.form.color1);
        var param = {
          id: id,
          gridArea: JSON.stringify(this.path),
          color: this.form.color1,
        };
        var res = await putGridSignin(param);
        console.log(res);
        //修改完在获取一次数据
      ...
      if (this.gridtype == "2") {
        //网格
        var id = this.pustAllData;
        console.log(this.path, "网格区域设置");
        var param = {
          id: id,
          area: JSON.stringify(this.path),
          color: this.form.color1,
        };
        var res = await putAreaManage(param);
        console.log(res);
        //修改完在获取一次数据
        this.mapdui.clearOverlays();
        //获取数据
      ...
      //    color: this.form.color1,
      //  });
      //  console.log(this.form.color1, "=======报存的眼神");
      //  this.getRegionalDivision();
      //  //location.reload();
      //}
    },
    ...
    //  await shanchu_region({ id: rows[index].id });
    //  await this.mapdui.clearOverlays();
    //  await this.getRegionalDivision();
    ...
    async baiduMapInit() {
      ...
      var mapLatitude = this.maplat ? this.maplat : "33.002475";
      var mapLongitude = this.maplng ? this.maplng : "112.504298";
      ...
      var drawingManager = new BMapLib.DrawingManager(that.mapdui, {
        ...
        drawingToolOptions: {
          ...
          offset: new BMap.Size(0, 40), //偏离值  左，上
      ...
      //drawingManager.addEventListener("markercomplete", async function (e) {
      //  console.log(e, "点");
      //});
      var that = this;
      drawingManager.addEventListener("overlaycomplete", async function (e) {
        if (e.drawingMode == "marker") {
          ...
          console.log(e);
          var markerPosition = e.overlay.getPosition();
          //console.log(markerPosition, "点击添加一个点");
          markerPosition.lng = markerPosition.lng.toString();
          markerPosition.lat = markerPosition.lat.toString();
          //console.log(markerPosition, "转换完成的数据");
          //添加数据域
          //修改====添加打卡点坐标
          var id = that.pustAllData;
          var param = {
            id: id,
            longitude: markerPosition.lng,
            latitude: markerPosition.lat,
          };
          var res = await putPunchPoint(param);
          console.log(res);
          //await that.setPointList(markerPosition);
          ////获取数据
          await that.getPointList();
          //return;
          //修改完在获取一次数据
          that.mapdui.clearOverlays();
          //获取数据
          that.getRegionalDivision();
          that.getPointList();
          that.getAllFenceArea();
          that.getData22();
        } else {
          console.log(e, "e+++++");
          newOverlay = e;
          ...
          //         closeBtn: 0,
          //           btn2:function(){
          //             that.mapdui.removeOverlay(e.overlay);
          ...
          //         await baocun_region({"name":value,"area":JSON.stringify(path)})
    ...
    async getAllFenceArea() {
      var res = await getFenceArea();
      console.log(res, "网格数据");
      var liss = res.result.records;
      //this.handleArea(liss);
      var that = this;
      liss.forEach((item, i) => {
        if (item.area) {
    ...
    ////获取数据----设置打卡点的坐标
    //async setPointList(form) {
    //  //var aaa = {
    //  //  lng: "112.50712766235638",
    //  //  lat: "33.00407988607854",
    //  //};
    //  await shezhi_point(form);
    //},
    //获取数据-----获取打卡点的坐标数据
    async getPointList() {
      var result = await getPunchPoint();
      var pointList = result.result.records;
      console.log(pointList, "打卡点");
    ...
    //删除数据=====打卡点删除
    //async deleteDaka(index, val) {
    //  var form = val[index].id;
    //  console.log(form, "id的");
    //  await delete_point(form);
    //  await this.mapdui.clearOverlays();
    //  await this.getPointList();
    //  await this.getRegionalDivision();
    //},
    ...
    addAllData(a, b) {
      ...
      this.pustAllData = b.id;
      this.gridtype = b.type ? b.type : null;
    },
    //获取数据
    async getData22() {
      console.log("我执行了");
      var res = await getAllPunchTheClock();
      console.log(res, "所有打卡区域网格点的数据");
      this.tableData22 = res.result;
    ...
    lookAllData(a, b) {
      ...
      if (b.belongingArea) {
        ...
        if (!b.latitude) {
          ...
          var lat = b.latitude;
          var lng = b.longitude;
          this.mapdui.panTo(new BMap.Point(lng, lat));
        }
      } else if (b.gridArea) {
        console.log(b);
        var gridArea = JSON.parse(b.gridArea);
        var lat = gridArea[0].lat;
        var lng = gridArea[0].lng;
        this.mapdui.panTo(new BMap.Point(lng, lat));
      } else {
        this.$message({
          showClose: true,
  ...
  activated() {
    if (sessionStorage.getItem("caijiweizhi_date")) {
      setTimeout(() => {
        ...
        this.createMark_quan(point, 0, caiji_data, true);
  ...
  mounted() {
    ...
    //调试----点的坐标
    //this.$nextTick(this.getPointList());
    //PubSub.publish("nmhead");
    //var res = getAllPunchTheClock();
    //this.tableData22 = res.result;
    //console.log(res, "所有打卡区域网格点的数据");
  },
  created() {

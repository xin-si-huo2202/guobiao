    ...
    async fanjuisdss() {
      ...
      try {
        ...
        if (this.fankui_data.isrx) {
          res = await gyxyFaceWarning_editResponse({
            id: this.fankui_data.id,
            response: this.textarea,
          });
        } else {
          res = await gyxyPlateWarning_editResponse({
            id: this.fankui_data.id,
            response: this.textarea,
          });
        }
        if (this.fankui_data.isPhone) {
        ...
        if (res.result.indexOf("成功") != -1) {
          ...
          PubSub.publish("fankui_fanhui", this.textarea);
          Message({
            showClose: true,
            message: "反馈成功",
            type: "success",

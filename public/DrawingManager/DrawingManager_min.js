...
(function () {
    ...
    d.prototype._bindMarker = function () {
        ...
        var i = this,
            ...
            h = this._mask;
        var g = function (l) {
            const myIcon = new BMap.Icon('/DrawingManager/Aim.png', new BMap.Size(16, 16));

            // var k = new BMap.Marker(l.point, i.markerOptions);
            var k = new BMap.Marker(l.point, { icon: myIcon });

            //var maker_circle = new BMap.Circle(l.point, 500, this.circleOptions);
            j.addOverlay(k);
            overlays.push(k)


            //j.addOverlay(maker_circle);
            //maker_circle.enableEditing();

            //var p = i._calculate(maker_circle, l.point);

            //i._dispatchOverlayComplete(maker_circle, p)
            i._dispatchOverlayComplete(k)

            // maker_circle.addEventListener("lineupdate", function(e2){
            //     console.log(111)
            //     // var p = i._calculate(e2.target, l.point);
            //     // i._dispatchOverlayComplete(e2.target,p)
    ...
    d.prototype._bindCircle = function () {
        ...
        var g = function (p) {
            ...
            //清除
            var allclear = this._map.getOverlays()

            console.log(overlays, 'allclear====')
            for (var clear_i = 0; clear_i < overlays.length; clear_i++) {
                // if(allOverlay[clear_i].getLabel().content == "我是id=1"){
                // console.log(allclear[clear_i].content,'[clear_i].getLabel()666')
                console.log(overlays[clear_i], 'overlays[clear_i]')
                this._map.removeOverlay(overlays[clear_i]);
                // return false;
                // }
            }
            // debugger
            allclear.forEach((d1, i) => {


                if (d1) {
                    console.log($(d1._div).attr('class'))
                    if ($(d1._div).attr('class') == 'quanbox' || $(d1._div).attr('class') == 'loadingbox') {


                        this._map.removeOverlay(d1);
    ...
    d.prototype._bindclear = function () {
        var k = this,
            n = this._map,
            h = this._mask,
            i = null,
        ...
        // console.log(this._map.getOverlays(),'tttttt')
        // console.log(this._mask,'this._mask')
        // this._map.removeOverlay(this._map.getOverlays());
        var allclear = this._map.getOverlays()
    ...
    d.prototype._addGeoUtilsLibrary = function () {
        if (!BMapLib.GeoUtils) {
            ...
            g.setAttribute("src", "/DrawingManager/GeoUtils_min.js");
    ...
    function a(h, g) {
        ...
        this.defaultDrawingModes = [BMAP_DRAWING_MARKER, BMAP_DRAWING_CIRCLE, BMAP_DRAWING_POLYLINE, BMAP_DRAWING_POLYGON, BMAP_DRAWING_RECTANGLE, BMAP_DRAWING_CLEAR];
        if (g.drawingModes) {
    ...
    a.prototype._generalHtml = function (m) {
        ...
        for (var j = 0, g = this.drawingModes.length; j < g; j++) {
            ...
            console.log(this.drawingModes, 'this.drawingModes')
